...
void config()
{
    ...
    GetPrivateProfileString("Volume","Shot","50",buf[6],256,"config.ini");
    GetPrivateProfileString("Volume","Shot","1",buf[7],256,"config.ini");
    GetPrivateProfileString("Volume","Other charakters","40",buf[8],256,"config.ini");
    iniToSetup(buf);
}
